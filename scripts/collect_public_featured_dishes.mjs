...
const INPUT = path.join(DATA, 'public_pool_area1.js');
const OUTPUT = process.argv[2] || path.join(DATA, 'public_featured_dishes_auto.json');
const TIMEOUT_MS = Number(process.env.FEATURED_DISH_FETCH_TIMEOUT_MS || 7000);
const HOST_WORKERS = Math.max(1, Math.min(32, Number(process.env.FEATURED_DISH_HOST_WORKERS || 20)));
...
function loadPublicRows() {
  const sandbox = { window: {}, console };
  vm.createContext(sandbox);
  vm.runInContext(fs.readFileSync(INPUT, 'utf8'), sandbox, { filename: 'public_pool_area1.js' });
  return Array.isArray(sandbox.window.PUBLIC_OPEN_RESTAURANTS)
...
function sourceEligible(row) {
  ...
  if (/facebook\.com$|instagram\.com$|x\.com$|twitter\.com$|youtube\.com$/.test(host)) return false;
  return true;
}
...
function extractDishEvidence(pageText, sourceUrl) {
  if (!pageText || !SIGNATURE_MARKER.test(pageText)) return [];
  const dishes = [];
  const seen = new Set();
  for (const [pattern, [nameZh, fallbackJa]] of DISH_RULES) {
    const matches = [...pageText.matchAll(new RegExp(pattern.source, `${pattern.flags.includes('i') ? 'i' : ''}g`))];
    for (const match of matches) {
      ...
      if (!SIGNATURE_MARKER.test(windowText)) continue;
      ...
      dishes.push({
        ...
        evidenceText: windowText.slice(0, 220)
      });
      break;
    }
...
function menuLinks(html, baseUrl) {
  ...
  while ((match = linkPattern.exec(html))) {
    ...
    const raw = match[1];
    if (!MENU_LINK_MARKER.test(`${raw} ${label}`)) continue;
    try {
...
async function inspectRow(row) {
  const root = safeUrl(row.sourceUrl);
  if (!root) return { row, status: 'invalid_url', dishes: [] };
  ...
  for (let index = 0; index < urls.length && index < 3; index += 1) {
    ...
    try {
      ...
      const dishes = extractDishEvidence(pageText, url);
      if (dishes.length) return { row, status: 'matched', dishes, checkedUrl: url };
      if (index === 0) urls.push(...menuLinks(html, url));
    } catch (error) {
      errors.push(`${url}: ${error?.message || error}`);
    }
  }
  return { row, status: errors.length ? 'no_match_with_errors' : 'no_match', dishes: [], errors: errors.slice(0, 2) };
}

async function main() {
  const rows = loadPublicRows()
    .filter((row) => row.featuredDishConfidence === 'reference_hint')
    .filter(sourceEligible);

  const byHost = new Map();
  for (const row of rows) {
    const host = safeUrl(row.sourceUrl).hostname.toLowerCase();
  ...
  async function worker() {
    while (true) {
      ...
      const [, hostRows] = hostEntries[index];
      for (const row of hostRows) results.push(await inspectRow(row));
    }
  }
  ...
  const outputRows = matched.map((result) => ({
    identityKey: result.row.identityKey,
    name: result.row.name,
    cuisine: result.row.cuisine,
    sourceIdentityUrl: result.row.sourceUrl,
    checkedUrl: result.checkedUrl,
    featuredDishes: result.dishes
  }));
  const statusCounts = {};
  ...
  const payload = {
    ...
    checkedAt: CHECKED_AT,
    policy: {
      sourceEligibility: 'retained non-Google provider/official website URL',
      featuredDishRule: 'concrete dish term within 110 characters of recommendation/signature wording',
      maxSameHostMenuLinksFollowed: 2,
      paidGoogleDataApiCalls: 0
    },
    summary: {
      publicRowsInspected: rows.length,
      hostsInspected: hostEntries.length,
      matchedRestaurants: outputRows.length,
      featuredDishItems: outputRows.reduce((sum, row) => sum + row.featuredDishes.length, 0),
      statusCounts
    },
    rows: outputRows
  };
