...
function normalizeIdentityName(v){return String(v||'').normalize('NFKC').toLowerCase().replace(/[\s\p{P}\p{S}]+/gu,'');}
...
const tasks=[];
let skippedMissingRuntimeIdentityName=0,sourceAliasDiffers=0;
for(const r of official.rows||[]){
  if(r.reviewState!=='reviewed'||!noDishIds.has(r.googlePlaceId))continue;
  const runtimeRow=runtimeById.get(r.googlePlaceId);
  const identityName=String(runtimeRow?.name||'').trim();
  if(!runtimeRow||runtimeRow.nameKnown!==true||!identityName){skippedMissingRuntimeIdentityName+=1;continue;}
  const sourceOfficialName=String(r.officialName||'').trim();
  if(sourceOfficialName&&normalizeIdentityName(sourceOfficialName)!==normalizeIdentityName(identityName))sourceAliasDiffers+=1;
  for(const raw of r.menuUrls||[]){const url=htmlUrl(raw);if(url)tasks.push({googlePlaceId:r.googlePlaceId,name:identityName,sourceOfficialName:sourceOfficialName||null,url});}
}
...
const payload={schemaVersion:2,checkedAt:new Date().toISOString().slice(0,10),policy:{catalogTotal:2804,currentNoDishOnly:true,reviewedOfficialMenuUrlsOnly:true,toolIdentityNameSource:'runtime_catalog_name',sourceOfficialNameRole:'source_alias_only',sourceOfficialNameMayReplaceToolIdentity:false,embeddedJsonOnly:true,jsonLdExcluded:true,nameAndPriceSameObjectRequired:true,pageScriptsExecuted:false,networkApiCalls:0,paidGoogleDataApiCalls:0,recommendationPromotionAllowed:false,evidenceMutationAllowed:false},summary:{currentNoDish:noDishIds.size,sourceAliasDiffers,skippedMissingRuntimeIdentityName,htmlMenuTasks:tasks.length,pagesFetched:results.filter(r=>!r.error).length,errorPages:results.filter(r=>r.error).length,pagesWithEmbeddedJson:results.filter(r=>r.jsonScripts>0).length,evidenceRestaurants:new Set(evidenceRows.map(r=>r.googlePlaceId)).size,evidenceItems:evidenceRows.reduce((s,r)=>s+r.items.length,0)},rows:results};
