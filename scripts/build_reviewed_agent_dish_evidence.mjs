...
export function auditReviewCoverage(assignments, documents, { sourceQueueCommit } = {}) {
  ...
  for (const row of assignments) {
    if (!Object.values(LANES).includes(row.lane)) throw new Error('Assignment outside reviewed dish scope');
...
function main() {
  ...
  const reviewedScopes = new Set(documents.map(({ document }) =>
    `${LANES[document.marker]}:${Number(document.shard.slice(1))}`));
