...
const OUTPUT = process.argv[2] || path.join(ROOT, '_audit', 'hotpepper_unmatched_price_backed_headings.json');
const CONCURRENCY = Math.max(1, Math.min(16, Number(process.env.HOTPEPPER_UNMATCHED_CONCURRENCY || 8)));
const TIMEOUT_MS = Math.max(5000, Number(process.env.HOTPEPPER_UNMATCHED_TIMEOUT_MS || 10000));
const LIMIT = Math.max(0, Number(process.env.HOTPEPPER_UNMATCHED_LIMIT || 0));
...
function existingDishMatches(text) {
  const clean = String(text || '').replace(/\s+/g, ' ').trim();
  const names = new Set();
  if (/肉寿司/i.test(clean)) names.add('肉寿司');
  if (/鉄板焼(?:き)?そば|焼きそば|焼そば/i.test(clean)) names.add('炒面');
  for (const [pattern, nameZh] of DISH_RULES) {
    if (names.has(nameZh)) continue;
    if (clean.match(pattern)) names.add(nameZh);
  }
  return [...names];
}

function priceBackedHeadings(html, sourceUrl) {
  ...
  for (let i = 0; i < h4s.length; i += 1) {
    ...
    const existingMatches = existingDishMatches(heading);
    rows.push({
      heading,
      sourceUrl,
      priceContext: neighborhood.slice(0, 180),
      existingMatches
    });
  }
  return rows;
  ...
  try {
    ...
    for (const foodUrl of discoveredFoodLinks(root.html, root.finalUrl)) {
      try {
        ...
        result.headings.push(...priceBackedHeadings(food.html, food.finalUrl));
      } catch (error) {
...
for (const row of results) {
  for (const heading of row.headings) {
    const item = {
      ...
      sourceUrl: heading.sourceUrl,
      priceContext: heading.priceContext,
      existingMatches: heading.existingMatches
    };
    if (heading.existingMatches.length) matchedRows.push(item);
    else unmatchedRows.push(item);
  }
}
const uniqueUnmatched = [...new Set(unmatchedRows.map((row) => row.heading))];
const restaurantsWithUnmatched = new Set(unmatchedRows.map((row) => row.googlePlaceId));
const payload = {
  schemaVersion: 1,
  ...
  summary: {
    ...
    priceBackedHeadingRows: unmatchedRows.length + matchedRows.length,
    headingsAlreadyMatchedByExistingDictionary: matchedRows.length,
    unmatchedHeadingRows: unmatchedRows.length,
    uniqueUnmatchedHeadings: uniqueUnmatched.length,
    restaurantsWithUnmatchedHeadings: restaurantsWithUnmatched.size,
  ...
  diagnostics: results.map((row) => ({
    ...
    priceBackedHeadings: row.headings.length,
