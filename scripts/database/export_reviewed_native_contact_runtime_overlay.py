#!/usr/bin/env python3
"""Export reviewed retained Overture/OSM native metadata for public runtime.

This is an offline field-only overlay. It reuses the SQLite master's retained identity
collision model and native field validators. It never performs identity matching, never
calls a network/API, and never emits Google display payloads.

Runtime precedence is aligned with the master build:
  basic reviewed OSM practical -> retained/Hot Pepper practical -> official practical
  -> historical reviewed OSM practical for residual gaps;
  reviewed Overture phone -> public-web phone -> reviewed historical OSM phone.
"""
...
RULE_VERSION = "reviewed-native-contact-runtime-v2"
...
def _basic_osm_practical_rows(basics: dict, conflict_keys: set[str]):
    ...
    for row in basics.get("rows") or []:
        ...
        if not provider_id or not pid:
            continue
        if f"OpenStreetMap|{provider_id}" in conflict_keys:
            counts["binding_conflict"] += 1
            continue
        reviewed += 1
        ...
        if native_source_id in native_collisions:
            ...
            continue
        practical = bound_open_data._native_practical_claims(candidate)
        if not practical:
            counts["no_mappable_practical"] += 1
            continue
        output.append({
            "googlePlaceId": pid,
            ...
            "practicalClaims": practical,
            "sourcePracticalTags": candidate.get("sourcePracticalTags") or {},
            "sourceUrl": osm_identity.osm_url(candidate),
            "checkedAt": row.get("sourceCheckedAt") or "2026-09-05",
            "binding": {
                "state": "reviewed",
                "method": "retained_repository_basic_match",
                "matchLevel": row.get("matchLevel"),
            },
            "ruleVersion": bound_open_data.RULE_VERSION,
        })
        counts["emitted"] += 1
    output.sort(key=lambda item: (item["googlePlaceId"], item["providerId"]))
    return output, counts, reviewed


def _historical_osm_rows(id_set: set[str], conflict_keys: set[str], conflict_places: set[str]):
    ...
    for pid, provider_id, qc, imported_candidate in reviewed_rows:
        ...
        practical = osm_native._practical_claims(candidate)
        if practical:
            practical_output.append({
                **common,
                "practicalClaims": practical,
                "sourcePracticalTags": candidate.get("sourcePracticalTags") or {},
            })
            counts["practical_emitted"] += 1
        elif candidate.get("sourcePracticalTags"):
            counts["practical_tags_unmappable"] += 1
        ...
        if len(phones) == 1:
            ...
            counts["phone_emitted"] += 1
        elif len(phones) > 1:
            counts["multiple_native_phones_deferred"] += 1
        else:
            counts["no_valid_native_phone"] += 1

    phones_output.sort(key=lambda item: (item["googlePlaceId"], item["providerId"]))
    practical_output.sort(key=lambda item: (item["googlePlaceId"], item["providerId"]))
    counts["missing_candidate_pairs"] = len(missing_candidates)
    return phones_output, practical_output, counts, len(reviewed_rows)
...
def build_overlay() -> dict:
    ...
    osm_basic_practical, osm_basic_counts, reviewed_basic_osm = _basic_osm_practical_rows(
        basics, conflict_keys
    )
    osm_rows, osm_practical_rows, osm_counts, reviewed_osm = _historical_osm_rows(
        id_set, conflict_keys, conflict_places
    )

    all_rows = [*overture_rows, *osm_basic_practical, *osm_rows, *osm_practical_rows]
    if any(row["googlePlaceId"] not in id_set for row in all_rows):
        raise RuntimeError("native metadata overlay leaked a Place ID outside the frozen catalog")
    if any(row.get("binding", {}).get("state") != "reviewed" for row in all_rows):
        raise RuntimeError("native metadata overlay leaked a non-reviewed binding")
    ...
    return {
        ...
        "policy": {
            ...
            "basicOsmPracticalRule": bound_open_data.RULE_VERSION,
            "overturePhoneRule": overture.RULE_VERSION,
            "osmPhoneRule": osm_native.RULE_VERSION,
            "osmPracticalRule": osm_native.RULE_VERSION,
            "osmSingleNativePhoneRequired": True,
            "explicitOsmPracticalTagsOnly": True,
            ...
            "runtimeMutationAllowed": [
                "telephone", "telephoneSourceUrl", "telephoneCheckedAt", "telephoneProvider", "telephoneRuleVersion",
                "cardPaymentAvailable", "wifiAvailable", "barrierFree", "smokingPolicy"
            ],
            "telephoneRuntimePrecedence": ["Overture Maps", "public_web", "OpenStreetMap"],
            "practicalRuntimePrecedence": ["basic OpenStreetMap", "retained practical", "official practical", "historical OpenStreetMap"],
        },
        "summary": {
            "catalogTotal": 2804,
            ...
            "reviewedBasicOsmBindings": reviewed_basic_osm,
            "osmBasicPracticalRows": len(osm_basic_practical),
            "reviewedHistoricalOsmBindings": reviewed_osm,
            "osmTelephoneRows": len(osm_rows),
            "osmPracticalRows": len(osm_practical_rows),
            "overtureSkipped": {k: v for k, v in sorted(overture_counts.items()) if k != "emitted" and v},
            "osmBasicSkipped": {k: v for k, v in sorted(osm_basic_counts.items()) if k != "emitted" and v},
            "osmSkipped": {k: v for k, v in sorted(osm_counts.items()) if k not in {"phone_emitted", "practical_emitted"} and v},
        },
        "overtureRows": overture_rows,
        "osmBasicPracticalRows": osm_basic_practical,
        "osmRows": osm_rows,
        "osmPracticalRows": osm_practical_rows,
    }
