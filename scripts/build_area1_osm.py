...
def fetch_overpass():
 ...
 for ep in ENDPOINTS:
  try:
   req=urllib.request.Request(ep,data=data,headers={'User-Agent':'nekooweb-eat-static-builder/1.3'}); return json.loads(urllib.request.urlopen(req,timeout=210).read().decode())
...
def truthy(v): return None if v is None else str(v)
def quality_score(rec):
 fields=['address','openingHoursRaw','phone','website','cuisineRaw','brand','takeaway','delivery']
 return sum(1 for f in fields if rec.get(f))
def main():
 existing=curated_names(); raw=fetch_overpass(); out=[]; seen_osm=set(); seen_entity=set()
 for el in raw.get('elements',[]):
  ...
  if d>RADIUS_M+5 or key in seen_osm or norm(name) in existing:continue
  seen_osm.add(key)
  entity_key=(norm(name),round(float(lat),4),round(float(lng),4))
  if entity_key in seen_entity:continue
  seen_entity.add(entity_key)
  c=cuisine_for(t); addr=address(t); opening=t.get('opening_hours') or None
  phone=t.get('contact:phone') or t.get('phone') or None
  website=t.get('contact:website') or t.get('website') or None
  rec={'id':'osm-'+el.get('type','x')[0]+'-'+str(el.get('id')),'profile':'TOKYO','area':'地区1️⃣','name':name,'nameEn':t.get('name:en') or None,'cuisine':c,'cuisineRaw':t.get('cuisine') or None,'tags':[c],'distance':int(round(d/50)*50),'distanceMeters':int(round(d)),'lunch':None,'dinner':None,'dishes':['推荐菜待补充'],'closedNote':'休日信息见公开营业时间字段' if opening else '休日待核验','schedule':None,'openingHoursRaw':opening,'closedDays':[],'holidayNote':None,'phone':phone,'website':website,'brand':t.get('brand') or None,'operator':t.get('operator') or None,'takeaway':truthy(t.get('takeaway')),'delivery':truthy(t.get('delivery')),'outdoorSeating':truthy(t.get('outdoor_seating')),'wheelchair':truthy(t.get('wheelchair')),'smoking':truthy(t.get('smoking')),'paymentCash':truthy(t.get('payment:cash')),'address':addr,'postcode':t.get('addr:postcode') or None,'mapQuery':' '.join(x for x in [name,addr,'東京都'] if x),'lat':round(float(lat),6),'lng':round(float(lng),6),'source':'OpenStreetMap','sourceId':key,'sourceTimestamp':t.get('check_date') or t.get('survey:date') or None,'dataLevel':'base','hyakumeiten':False,'randomWeight':1}
  rec['dataQualityScore']=quality_score(rec); out.append(rec)
 out.sort(key=lambda x:(x['distanceMeters'],x['name']))
 OUT.write_text('// Auto-generated. Source: OpenStreetMap contributors (ODbL).\nwindow.RESTAURANTS.push(\n'+',\n'.join(json.dumps(r,ensure_ascii=False,separators=(',',':')) for r in out)+'\n);\n',encoding='utf-8')
 complete=sum(1 for r in out if r['dataQualityScore']>=4); with_hours=sum(1 for r in out if r.get('openingHoursRaw')); with_contact=sum(1 for r in out if r.get('phone') or r.get('website'))
 print(f'Generated {len(out)} Area 1 POIs within {RADIUS_M}m; quality>=4: {complete}; hours: {with_hours}; contact: {with_contact}')
 return 0
