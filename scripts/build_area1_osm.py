...
CENTER_LAT=35.6959; CENTER_LNG=139.7576; RADIUS_M=4500; MIN_REQUIRED=10000
...
def curated_names():
 ...
 return names
def fetch_overpass():
 q=f'''[out:json][timeout:240];(nwr(around:{RADIUS_M},{CENTER_LAT},{CENTER_LNG})["amenity"~"^(restaurant|fast_food|cafe|food_court)$"]["name"];nwr(around:{RADIUS_M},{CENTER_LAT},{CENTER_LNG})["shop"~"^(bakery|pastry|confectionery|deli|coffee|tea)$"]["name"];);out center tags;'''; data=urllib.parse.urlencode({'data':q}).encode(); last=None
 for ep in ENDPOINTS:
  try:
   req=urllib.request.Request(ep,data=data,headers={'User-Agent':'nekooweb-eat-static-builder/1.1'}); return json.loads(urllib.request.urlopen(req,timeout=270).read().decode())
...
def main():
 ...
 if len(out)<MIN_REQUIRED: print(f'WARNING: named real POIs below target {MIN_REQUIRED}; no synthetic records added.',file=sys.stderr)
 return 0
