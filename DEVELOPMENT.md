...
## 2026-09-15：餐厅分类与筛选方案（仅文档）

新增 [餐厅分类与筛选方案](CUISINE_FILTER_PLAN.md)，明确三个分类维度、同义词规则、同维度父子排除、复合与未知项处理、关键词候选边界，以及筛选变化后旧结果的处理方式。预算、距离与三家随机规则维持既有产品边界，已指出预算上限实现与区间重叠需求的差异，未宣称修复。

实施顺序为先做字典与筛选语义，再做关键词辅助；后续以固定小样例和手机操作验收，不为制定方案追加全量分析或采集。本次只更新方案及文档入口，不改 `app.js`、页面样式、原始数据、SQLite 或工作流，不手动发起数据重建和部署。线上功能仍以实际代码为准，后台数据处理独立继续。
