...
为了避免后续重复访问网页，Phase A 允许在同一次页面检查中**旁路记录**明确出现的 hours / lunch budget / dinner budget 原始 evidence candidate；这些 sidecar evidence 只保存证据，不在分类 review 中自动写入对应字段。后续 metadata central review 可以复用它们。\n\nS5 已完成第一批完整生命周期：8/8 worker review 后，经独立 central review 保留 4 条 `accepted_evidence`、1 条 candidate、3 条 blocked，paid Google Data API = 0。4 条 accepted 由 deterministic reviewed-truth builder 写入 accepted entity overlay；candidate/blocked 使用 terminal cooldown，只有 cooldown 到期或 assignment-style source fingerprint 改变时才重新激活。 maintained preview 实测 accepted **1,261 → 1,265（+4）**、unknown **161 → 157（-4）**、coverage **88.68% → 88.96%**；4 条 terminal non-accepted 被 deferred，source-changed reactivation = 0，active bound-source **100 → 92**，S5 active assignment **8 → 0**。 S6 也已完成 central review：8/8 worker review 的 3 条 `accepted_evidence` proposal 全部保留，另有 1 candidate、1 no_evidence、3 blocked；reviewed truth 现累计 16 条，其中 accepted 7 条。S6 非 accepted terminal 状态沿用同一 cooldown/source-change lifecycle。 maintained preview 实测 accepted **1,265 → 1,268（+3）**、unknown **157 → 154（-3）**、coverage **88.96% → 89.17%**；terminal deferred **4 → 9**，active bound-source **92 → 84**，S6 active assignment **8 → 0**。 S7 已完成 central review：9/9 worker review 的 4 条 `accepted_evidence` proposal 全部保留，另有 2 no_evidence、3 blocked；reviewed truth 现累计 25 条，其中 accepted 11 条。S7 非 accepted terminal 状态沿用同一 cooldown/source-change lifecycle。 maintained preview 实测 accepted **1,268 → 1,272（+4）**、unknown **154 → 150（-4）**、coverage **89.17% → 89.45%**；terminal deferred **9 → 14**，active bound-source **84 → 75**，S7 active assignment **9 → 0**。 S2 已完成 central review：11/11 worker proposal 中 3 条 `accepted_evidence` 全部保留，另有 2 candidate、1 no_evidence、5 blocked；KANDA SQUARE 的 mixed-use facility identity conflict 继续 fail-closed。reviewed truth 现累计 36 条，其中 accepted 14 条；accepted overlay 14 条。maintained PR validation 实测 accepted **1,272 → 1,275（+3）**、unknown **150 → 147（-3）**、coverage **89.45% → 89.66%**；cuisineStyle **519 → 521**、foodType **265 → 266**、venueType 保持 **595**，多维命中保持 **96**。active bound-source **75 → 64**、explicit bound links **84 → 73**，S2 active assignment **11 → 0**。
