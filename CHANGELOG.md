...
- 1,613 unique Google-verified food-related business entities remained after Place ID deduplication and strict <=1.2km filtering.
- Generated `data/area1_google_places.json` and browser-ready `data/area1_google.js` were committed to `main`.

### Google-first frontend migration
- Fixed the remaining production integration gap by loading `data/area1_google.js` in `index.html`.
- Production recommendation eligibility now comes only from Google-first records (`source: Google Places`, `googleStatus: verified`) within the strict 1.2km boundary.
- Historical manual/bulk/OSM datasets remain available only as enrichment inputs and cannot independently enter the recommendation pool.
- Frontend enrichment merges compatible legacy/manual metadata onto Google entities by exact Place ID where possible, otherwise by compatible normalized name/location.
- Google-native name/address/coordinates/status/type remain authoritative during enrichment.
- Google Maps navigation now prefers Place ID queries before exact Google Maps URI/name-address fallbacks.
- Manual curated Google correction layer is loaded last so explicit corrections win over generated overlays.

### Metadata completeness UI
- Restaurant cards now display a `Google已核验` badge.
- Google temporary-closure state is surfaced as a warning rather than silently hidden.
- Missing budget, representative dishes, and opening/holiday metadata are explicitly shown as `待补充` rather than fabricated.
- Each card exposes a concise metadata-completeness line.
- Comparison table includes metadata completeness status.
- Database statistics were replaced with production-centric metrics:
  ...
  - 百名店 count
- Legacy OSM `verified/rejected/pending` counts are no longer presented as production database quality metrics.

### Generated verification integration
- Fixed an earlier integration gap where `data/google_entities.generated.js` was generated by Actions but not loaded by the webpage.
- Generated Google verification data is retained for source-matching/audit compatibility, while Google-first Area1 data is now the actual production universe.
...
- Updated `ARCHITECTURE.md` to document Google-first production identity, source-enrichment behavior, legacy verifier role, current frontend migration and remaining canonical precompile debt.
- Updated `DEVELOPMENT.md` to mark Google discovery and production-eligibility frontend migration complete, and move the active focus to metadata enrichment, coverage auditing and payload cleanup.
...
- precompiled canonical enrichment dataset (`area1_verified.js` or equivalent)
- Tabelog enrichment completeness audit
- 百名店 identity audit against Place IDs
- OSM/Tabelog cross-source coverage report
- frontend payload cleanup after enrichment precompile
